...
export const migrateToCohortCentric = async () => {
  ...
  try {
    ...
    console.log(
      "📊 Step 1: Updating cohorts with track statistics and settings...",
    );
    ...
    for (const cohort of cohorts) {
      ...
      for (let i = 0; i < cohort.tracks.length; i++) {
        ...
        console.log(
          `  Track ${trackId}: ${applications.length} applications, ${students.length} students`,
        );
      }
      ...
      if (!user.trackAssignments || user.trackAssignments.length === 0) {
        ...
        if (user.role === "student") {
          ...
          for (const trackId of oldTracks) {
            ...
            const cohortsWithTrack = await Cohort.find({
              "tracks.track": trackId,
            });

            for (const cohort of cohortsWithTrack) {
              const cohortTrack = cohort.tracks.find(
                (ct) => ct.track.toString() === trackId.toString(),
              );
              if (
                cohortTrack &&
                cohortTrack.mentors.includes(user._id as any)
              ) {
                newAssignments.push({
        ...
        if (newAssignments.length > 0) {
          ...
          await user.save();
          console.log(
            `✅ Migrated ${newAssignments.length} track assignments for user: ${user.email}`,
          );
        }
      }
    }
    ...
    for (const slot of interviewSlots) {
      ...
      if (
        ...
        await slot.save();
        console.log(
          `✅ Updated interview slot ${slot._id} to use single track`,
        );
      }
    }
    ...
    const validApplications = await Application.countDocuments({
      track: { $exists: true },
    });
    console.log(
      `Applications: ${validApplications}/${applicationsCount} have valid track references`,
    );

    // Check that all streams have valid tracks
    const streamsCount = await Stream.countDocuments({});
    const validStreams = await Stream.countDocuments({
      track: { $exists: true },
    });
    console.log(
      `Streams: ${validStreams}/${streamsCount} have valid track references`,
    );
    ...
    console.log(
      `Tasks: ${validTasks}/${tasksCount} have valid track references`,
    );

    // Check that all materials have valid tracks
    const materialsCount = await Material.countDocuments({});
    const validMaterials = await Material.countDocuments({
      track: { $exists: true },
    });
    console.log(
      `Materials: ${validMaterials}/${materialsCount} have valid track references`,
    );

    console.log("\n✅ Migration completed successfully!");
    console.log("\n📋 Migration Summary:");
    console.log(
      `- Updated ${cohorts.length} cohorts with enhanced track statistics`,
    );
...
const runMigration = async () => {
  try {
    const DATABASE_URI =
      process.env.DATABASE_URI || "mongodb://localhost:27017/uptick-lms";
