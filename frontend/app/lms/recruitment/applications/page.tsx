...
export default function ApplicationsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'assessment_submitted': return 'bg-blue-100 text-blue-800';
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {statuses.slice(1).map(status => {
            const count = applications?.filter((app: any) => app.status === status.value).length || 0;
            return (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                        ...
                        <div>
                          ...
                          <span className="ml-1">{formatDate(application.updatedAt)}</span>
                        </div>
                      </div>
                    </div>
                    
                    <div className="flex gap-2">
                      <Link href={`/lms/recruitment/applications/${application._id}`}>
