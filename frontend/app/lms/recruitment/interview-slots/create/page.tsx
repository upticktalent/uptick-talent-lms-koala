...
export default function CreateInterviewSlotsPage() {
  ...
  // Fetch tracks
  const { response: tracksData } = useFetch(() => trackService.getTracks());
  const { response: cohortData } = useFetch(() => cohortService.getCurrentActiveCohort());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'bulk') {
        ...
        console.log('Submitting bulk form:', bulkForm);
        await interviewSlotService.createSlots({
          mode: 'bulk',
          ...bulkForm, cohortId: cohortData?._id
        });
        ...
        console.log('Submitting manual slots:', manualSlots);
        await interviewSlotService.createSlots({
          mode: 'manual',
          slots: manualSlots, cohortId: cohortData?._id
        });
