...
export default function StreamPage() {
  ...
  const { user } = useUser();
  const { currentCohort, loading: cohortLoading } = useCohortContext();
  const cohortId = useCohortId();
  const cohortServices = useCohortAwareServices(cohortId);

  // Get track within current cohort
  ...
  useEffect(() => {
    if (currentCohort && track && cohortId) {
      fetchStreamData();
    }
  }, [currentCohort?._id, track?._id, cohortId]);

  const fetchStreamData = async () => {
    if (!track || !currentCohort || !cohortServices) return;

    try {
      setFetchingStreams(true);

      // Get streams for this cohort and track
      const streamsResponse: any = await cohortServices.streams.getAll(
        track._id,
        {
          page: 1,
          limit: 50,
      ...
      const tasksResponse: any = await cohortServices.tasks.getAll(track._id, {
        page: 1,
        limit: 20,
      });
      ...
      if (user?.role === "student") {
        // Upcoming tasks are already included in the tasks response
        const upcomingTasksFromResponse =
          tasksResponse.data.tasks?.filter(
            (task: any) => new Date(task.dueDate) > new Date()
          ) || [];
        setUpcomingTasks(upcomingTasksFromResponse.slice(0, 5));
      }
    } catch (error) {
  ...
  const allFeedItems = [
    ...streams.map((stream) => ({
      ...stream,
      feedType: "stream",
      author: `${stream.createdBy?.firstName || "Unknown"} ${
        stream.createdBy?.lastName || "User"
      }`,
    ...
    ...tasks.map((task) => ({
      ...task,
      feedType: "task",
      author: `${task.createdBy?.firstName || "Unknown"} ${
        task.createdBy?.lastName || "User"
      }`,
  ...
  // Now we can safely do early returns after all hooks are called\n  if (cohortLoading || loading || fetchingStreams) return <Loader />;\n  if (!currentCohort || !track) return null;
              ...
              <CardContent>
                ...
                <div className="flex justify-end">
                  <Button
                    ...
                    onClick={() =>
                      (window.location.href = `/lms/track/${slug}/classwork`)
                    }
                  >
                    View all
                  </Button>
                    ...
                    {(user?.role === "mentor" || user?.role === "admin") && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              handleDelete(item._id, item.feedType)
                            }
                    ...
                    <CardContent className="pt-0 pb-3 sm:pb-4 px-3 sm:px-4 ml-0 sm:ml-14">
                        ...
                        <div className="mt-3 space-y-2">
                          ...
                          {item.attachments.length > 2 && (
                            <p className="text-xs text-gray-500 pl-2">
                              +{item.attachments.length - 2} more attachment
                              {item.attachments.length - 2 !== 1 ? "s" : ""}
                            </p>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-4 pt-3 sm:pt-4 border-t mt-3 sm:mt-4">
                        ...
                        >
                          ...
                          {item.reactions?.filter((r: any) => r.type === "like")
                            .length || 0}
                        </Button>
                        <Button
                          variant="ghost"
                        ...
                        >
                          ...
                          {item.reactions?.filter((r: any) => r.type === "love")
                            .length || 0}
                        </Button>
                        <Button
                          variant="ghost"
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="flex items-center gap-4">
              ...
              {viewingItem?.type && (
                <Badge
                  className={getTypeColor(viewingItem.type)}
                  variant="secondary"
                >
                  {viewingItem.type}
                </Badge>
