...
console.log(track)
  const [streamItems, setStreamItems] = useState<StreamItem[]>([
    ...
    },
    {
        ...
        date: '2 days ago',
        content: 'Welcome to the course! Please make sure to review the syllabus and join the Slack channel.'
    }
  ]);
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent>
              ...
              <div className="flex justify-end">
                <Button variant="link" className="p-0 h-auto text-sm text-blue-600 font-medium">
                    View all
                </Button>
              </div>
            </CardContent>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="space-y-4">
            {streamItems.map((item) => (
                <Card key={item.id} className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                    <CardHeader className="p-4 flex flex-row items-center justify-between space-y-0">
                        <div className="flex items-center gap-4" onClick={() => setViewingItem(item)}>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${item.type === 'assignment' ? 'bg-blue-600 text-white' : 'bg-purple-100 text-purple-600'}`}>
                                {item.type === 'assignment' ? <FileText className="w-5 h-5" /> : <MessageSquare className="w-5 h-5" />}
                            </div>
                            <div>
                                <h3 className="text-sm font-medium text-gray-900">
                                    <span className="font-semibold">{item.author}</span> posted a new {item.type}: <span className="font-semibold">{item.title}</span>
                                </h3>
        ...
        <DialogContent>
            <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                    {viewingItem?.type === 'assignment' ? <FileText className="w-5 h-5 text-blue-600" /> : <MessageSquare className="w-5 h-5 text-purple-600" />}
                    {viewingItem?.title}
                </DialogTitle>
                <DialogDescription>
