...
export default function TrackGradesPage() {
  ...
  const { 
    ...
    if (track?._id) {
      return trackService.getTrackGrades(track._id, isStudent ? user?._id : undefined);
    }
  ...
  const mockGrades = isStudent ? [
    {
      ...
      score: 85,
      maxScore: 100,
      feedback: 'Great work on the responsive design! Consider improving the color scheme for better accessibility.',
      gradedBy: 'John Smith',
    ...
    },
    {
      ...
      score: 92,
      maxScore: 80,
      feedback: 'Excellent implementation! You went above and beyond with the advanced features.',
    ...
    },
    {
      ...
      score: 78,
      maxScore: 100,
      feedback: 'Good understanding of the basics. Review CSS Grid and Flexbox concepts.',
      gradedBy: 'John Smith',
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-blue-600">
                  {Math.round(mockGrades.reduce((acc, g) => acc + (g.score / g.maxScore) * 100, 0) / mockGrades.length)}%
                </div>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-purple-600">
                  {mockGrades.reduce((acc, g) => acc + g.score, 0)} / {mockGrades.reduce((acc, g) => acc + g.maxScore, 0)}
                </div>
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${getGradeBadge(grade.score, grade.maxScore)}`}>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-green-600">
                  {Math.round(mockGrades.reduce((acc: number, s: any) => acc + s.average, 0) / mockGrades.length)}%
                </div>
              ...
              <Card key={index}>
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                      <div className={`text-xl font-bold ${getGradeColor(student.average, 100)}`}>
                        {student.average.toFixed(1)}%
                      </div>
                      <span className={`px-2 py-1 text-xs rounded-full ${getGradeBadge(student.average, 100)}`}>
                        Overall
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {student.grades.map((grade: any, gradeIndex: number) => (
                      <div key={gradeIndex} className="bg-gray-50 rounded-lg p-3">
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-700">{grade.assignment}</span>
                          <span className={`text-sm font-bold ${getGradeColor(grade.score, grade.maxScore)}`}>
                            {grade.score}/{grade.maxScore}
                          </span>
                        </div>
